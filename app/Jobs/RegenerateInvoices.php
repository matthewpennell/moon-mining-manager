...
class RegenerateInvoices implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $debtors = Miner::select('eve_id')->where('amount_owed', '>=', 1)->whereRaw($whitelist_whereRaw)->whereRaw('eve_id NOT IN (SELECT miner_id FROM invoices WHERE DATE(created_at) = "' . $last_monday . '")')->get();
        Log::info('RegenerateInvoices: found ' . count($debtors) . ' miners who should have received an invoice but did not');
        $delay_counter = 0;

        foreach ($debtors as $miner)
        {
            GenerateInvoice::dispatch($miner->eve_id, $delay_counter * 20);
            Log::info('RegenerateInvoices: dispatched job to generate invoice for miner ' . $miner->eve_id . ' and send mail ' . ($delay_counter * 20) . ' seconds later');
            $delay_counter++;
        }
